...
> **v0.7.0 是破坏性变更**：路由级 IP 名单从 `acl.mode` 二选一改成 `allow` / `deny` 两份，
> 并新增对所有入口（含管理端口）生效的 `global_ip_deny`。用旧写法的配置**进程会拒绝启动**，
> 升级前先看[从 v0.6.x 升级](#从-v06x-升级破坏性变更)。
>
> **v0.6.0 也是破坏性变更**：管理控制台改为**用户名 + 密码**登录，并且**本机访问也不再免认证**。
> 见[从 v0.5.x 升级](#从-v05x-升级破坏性变更)。
>
> **控制台页签在 v0.7.1 重新分了一次（不涉及配置格式，升级无需改文件）**：三份 IP 名单
> 从「配置」页和各路由的表单里搬到单独的 **「IP 名单」** 页签，判定顺序和命中测试也在那一页。
> 见[管理控制台](#管理控制台网页版)。
...
| **路由** | 路由列表（三级匹配规则、实时请求数、熔断状态），一键启停；新建 / 编辑 / 删除；表单分「基础 · 转发 · 限流 · 熔断 · 认证」五组。**名单不在这里改**，表单里只回显条数并指路 |
| **IP 名单** | 三份名单的统一入口：全局黑名单、每条路由的白名单与黑名单，都在这一页编辑；外加**命中测试**（填地址 / 测我自己 / 选一条路由）。见 [IP 名单（三份，统一管理）](#ip-名单三份统一管理) |
...
> **为什么名单单独占一个页签。** 三份名单共用同一条判定链（见 [IP 名单](#ip-名单三份统一管理)），
> 拆在「配置」页和每条路由的表单里分别改，改之前得先想清楚「这个地址的问题该去哪一页查」。
> 现在它们在同一页上，判定顺序就写在页面上；路由表单只负责回显「这条路由配了几条」并给你个入口。
...
| `/_goproxy/config` | PATCH | 改 `default_ports` / `access_log` / `trusted_proxies` / `global_ip_deny` / `admin_token` / `admin_users` |
| `/_goproxy/acl/test` | GET / POST | **命中测试**：拿一个地址跑一遍三层名单，看它会被哪一层、哪条规则拦下。只读，不改状态 |
...
> **v0.7.0 起写法变了。** 旧版的 `acl.mode` + `acl.cidrs`（二选一）已被移除，
> 进程启动时会直接报错并给出迁移映射。见 [从 v0.6.x 升级](#从-v06x-升级破坏性变更)。

名单一共三份，判定顺序**固定**：

```
① 全局黑名单 global_ip_deny  → 命中即 403（对所有入口生效，含管理端口）
② 路由白名单 acl.allow       → 已配置但没命中即 403
③ 路由黑名单 acl.deny        → 命中即 403
④ 都没拦下                    → 放行
```

这个顺序就是全部规格，不需要再记别的规则。两个容易搞错的地方：

- **白名单一旦配置就只有一个含义：只允许名单内的地址。** 它是在收紧范围，不是在额外放行，
  所以不存在「和黑名单谁优先」的问题 —— allow 永远先判。没配 allow 才是「不限制来源」。
- **全局黑名单不可被豁免。** 一个地址即使出现在某条路由的白名单里，只要命中全局黑名单，
  照样 403。②③ 是路由级的，只在那条路由上生效；① 是不分路由的。

#### 在控制台里改

三份名单**在同一页**：控制台的 **「IP 名单」** 页签（紧挨「路由」，见 [管理控制台](#管理控制台网页版)）。

| 想改什么 | 在哪 |
|---|---|
| ① 全局黑名单 | 页面上半部分的文本框，改完点「保存并生效」 |
| ② ③ 某条路由的两份名单 | 下方路由表里那一行的「编辑名单」，展开后同时编辑白名单与黑名单 |
| 想先验证一下再保存 | 同页的**命中测试**面板 |

判定顺序（上面那四行）就印在这一页上，不用来回翻。两点和部署安全相关：

- **路由表单里不再编辑名单。** 「路由」页的表单只回显「这条路由配了几条」并指路到这里，
  避免同一份名单有两个入口。保存路由时会把原有名单原样带回（PUT 是整条替换），不会顺手抹掉。
- **清空一份名单要显式表达。** 名单是「合并语义」写进去的（PATCH 只改你提交的键），
  ...
  "routes": [
    {
      "id": "office-only",
      "acl": {
        "allow": [{ "cidr": "10.0.0.0/8", "note": "办公网" }, "192.168.0.0/16"],
        "deny": [{ "cidr": "10.0.0.66", "note": "老是异常流量" }]
      }
    }
  ]
}
...
`allow` 写成空数组会**直接报错**：那等于「谁都不许进」，几乎必然是配错了。
要「不限制来源」就把 `allow` 整个去掉，不要留个空数组。

#### 命中测试：保存之前先看一眼

三份名单有明确的先后顺序，光盯着配置列表很难在脑子里推出结果 ——
特别是同一个地址既出现在白名单里、又命中某层黑名单的时候。所以有个只读接口：

```bash
# 拿一个地址 + 一条路由跑一遍，看它会被哪一层、哪条规则拦下
...
`decision.steps` 则把三层逐条回放（哪层配了、有没有命中、命中的是哪条规则）：

```json
{
  "decision": {
    "ip": "10.0.0.66", "allowed": false,
    "layer": "黑名单", "rule": "10.0.0.66", "note": "老是异常流量",
    "reason": "acl_route_deny",
    "message": "命中本路由黑名单 10.0.0.66（老是异常流量）。",
    "steps": [
      { "layer": "全局黑名单", "configured": true,  "matched": false, "detail": "2 条规则，未命中" },
      { "layer": "白名单",     "configured": true,  "matched": true,  "rule": "10.0.0.0/8",
        "note": "办公网", "detail": "在白名单内，继续判黑名单" },
      { "layer": "黑名单",     "configured": true,  "matched": true,  "rule": "10.0.0.66",
        "note": "老是异常流量", "detail": "命中，判定结束" }
    ]
  },
  "self": false,
  "route_name": "只允许办公网访问，但其中一台机器单独拉黑"
}
```
...
> 顺带说清两种 409 的区别，它们的处理截然相反：`self_lockout` 是**你提交的东西本身有问题**，
> 界面必须保留输入；`revision_mismatch` 是**你手上的副本过期了**（别人先改过），
> 界面必须丢弃输入、重读最新配置。控制台按错误码区分这两者，别把它们混成一个「保存失败」。
...
同一个「被 IP 名单拦了」在日志和指标里现在是三个不同的标签，因为处理方式完全不同：

| `blocked` / `reason` | 含义 | 通常该怎么办 |
|---|---|---|
| `acl_global_deny` | 命中全局黑名单 | 去 `global_ip_deny` 里解除 |
| `acl_route_allow_miss` | 该路由配了白名单，但地址不在名单内 | 补一条 `acl.allow`，或者把白名单去掉 |
| `acl_route_deny` | 命中该路由的黑名单 | 去该路由的 `acl.deny` 里解除 |
...
| `acl.go` | 三层 IP 名单：`global_ip_deny` / 路由 `allow` / 路由 `deny`，统一由 `decideIP` 判定并产出可回放的 `Steps` |
