...
以及控制台「配置」页多了个**命中测试**，改名单之前可以先拿地址试一遍
...
| **路由** | 路由列表（三级匹配规则、实时请求数、熔断状态），一键启停；新建 / 编辑 / 删除；表单分「基础 · 转发 · 限流 · 熔断 · 认证 · IP 名单」六组（白名单与黑名单两份可以并存） |
| **日志** | 最近 500 条访问记录，按状态 / 路由 / 方法 / 关键字过滤，被拦截的请求单独标色；SSE 实时追加，可暂停、可跟随滚动 |
| **配置** | `default_ports` / `access_log` / `trusted_proxies` / `global_ip_deny` 的读写与手动重载，外加 **IP 名单命中测试**（填地址 / 测我自己 / 选一条路由）；`admin_users` 与 `admin_addr` 只读并说明原因 |
...
再来这里验证。控制台的「配置」页有同一个工具（填地址 / 「测我自己」/ 选一条路由）。
...
拒绝时**磁盘上的配置不会被改动**，控制台读到失败的提示后会自动重新拉一次配置，
把界面拉回真实状态。
