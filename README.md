...
### 日志里的 IP 地域（可选，qqwry.dat）

日志页每条记录的客户端 IP 下面会显示它的归属地（国家 / 省 / 市）。一屏陌生 IP 摆在面前
却说不出从哪来，排查就只能靠外网查 —— 这一行字省的就是那一步。

**这是一个可选功能**：地域库是个外部数据文件，没有它日志页显示「未启用」，
其余一切照旧。
...
| | |
|---|---|
| 数据 | **纯真 IP 库**（qqwry 格式），一个 `qqwry.dat` 文件，当前这份约 26 MB、150 万条索引 |
| 版本 | 文件里自带版本标记（形如 `纯真网络 2026年09月02日IP数据`）。日志页顶部会显示**文件时间和条数**，一眼能看出数据新不新 |
| 放哪 | 按顺序找：`-qqwry <路径>` / `GOPROXY_QQWRY` 显式指定 → **配置库同目录** → 可执行文件同目录 → 当前工作目录。install.sh 装的实例就是 `/etc/goproxy/qqwry.dat` |
| 获取 | 从纯真官网或其镜像下载免费版，解出 `qqwry.dat` 放到上面任一位置即可 |

**不进仓库**：`.gitignore` 里挡着 `qqwry.dat`。26 MB 的二进制数据跟着仓库走既撑大
每次 clone，又会在几天内过期。

#### 更新时机

库不是 embed 进去的，也不整份载进内存 —— 它是个**运行时文件**，用 `ReadAt` 按需随机读
（一次查询只读十几条索引加两三个字符串，26 MB 不占常驻内存）。所以换库不需要重新编译：

```bash
# 用新版覆盖它，然后让进程重新读一次（不必重启）
cp qqwry-新.dat /etc/goproxy/qqwry.dat
curl -u admin:xxx -X POST http://127.0.0.1:9080/_goproxy/reload   # 或 sudo systemctl restart goproxy
```

重载时会比对文件的 **mtime 与大小**，没变就什么都不做（已算好的缓存继续用）。
变了才重新打开，并且缓存随新库一起换掉 —— 不会出现「一半新地名一半旧地名」。

查询结果另有一层进程内缓存（4096 条，满了丢一半）。缓存只是省下重复的文件读，
丢了下次再读一遍，不影响正确性。

#### 显示什么，以及查不到时显示什么

「查不到」有好几种原因，它们是**不同的事**，所以显示也不同 —— 全塞进一句「未知」
会让人以为功能坏了：

| 情况 | 显示 | 说明（鼠标悬停） |
|---|---|---|
| 查到 | `江苏 南京` / `美国 加利福尼亚州 圣克拉拉` | 库里原文 + ISP，并注明来自本地 qqwry.dat |
| 内网 / 回环 / 保留地址 | `局域网` | 这是**确定的结论**（库不覆盖这类地址），不是查询失败 |
| IPv6 地址 | `IPv6` | qqwry 只有 IPv4，这条没有归属地 |
| 库在、但这段没收录 | `未收录` | 数据边界，不是故障 |
| 来源地址不是合法 IP | `地址不合法` | 无法查询 |
| 没放地域库 | `未启用` | 提示把 qqwry.dat 放到配置库同目录或用 `-qqwry` 指定 |
| 读库出错 | `查询失败` | 带上具体原因 |

国内地址显示成「省 市」（不必每行都写「中国」，直辖市的省市相同也去重）；
国外地址带上国家。**省市切分是启发式的**（库里给的是一个用 `–` 连起来的层级串，
层级数不固定），所以原始串一直保留在悬停提示里 —— 切错了要能看见原话。

#### 地域与 IP 的对齐

地域是**挂在同一条日志记录上**跟着出去的（`LogEntry.ip_geo`），不是前端拿一份
IP 列表去换一份地域列表再按顺序贴上去 —— 后者只要两边的顺序或长度有一点不一致
就会整列错位，而且看起来还挺正常。服务端在**出站时**才补地域（环形缓冲里那份
始终不带），所以日志写入这条热路径不会因为地域功能多一次文件读。
...
| `scripts/e2e_ipgeo.py` | 日志 IP 地域的端到端自检（见下，没有 qqwry.dat 时自动跳过） |
...
# IP 地域的端到端：用**真实的 qqwry.dat**（26 MB）验解析器。合成库只能证明
# 「解析器符合我理解的格式」，证明不了拿真库也能读出对的地名 —— 这里断言的是
# 8.8.8.8 -> 美国、114.114.114.114 -> 江苏 南京 这类常识。
# 同时验：/logs 与 SSE 两条路都带地域、内网/IPv6/未收录分开显示、
# 没有地域库时只降级不影响其它功能。没有 qqwry.dat 时跳过。
python scripts/e2e_ipgeo.py
```
