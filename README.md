...
**升级前会先做一次配置预检**：拿即将装上去的那个二进制**只读地**试读一次现有配置库，
读不了就直接拒绝并打印迁移映射 —— 而不是装上去、然后服务起不来。三条路都覆盖了
（`install.sh`、控制台、`-upgrade-apply`）。想单独确认可以手动跑
`goproxy -c <配置库> -config-check`；确实要带着不兼容的配置硬上时，控制台有
「仍然安装（强制）」，命令行的 `-upgrade-apply` 有 `-upgrade-force`。
