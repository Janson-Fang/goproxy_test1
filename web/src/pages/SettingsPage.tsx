...
export function SettingsPage({ onChanged }: { onChanged: () => void }) {
  ...
  // IP 名单（三份）不在这里维护 —— 它们搬到了「IP 名单」页签。
  // 页面里只留一句指路，见下面那个 Note。
      ...
      >
        <div className="stack">
          <div className="grid2">
            ...
            {/*
              IP 名单从这里搬走了（三份都搬）。放在「配置」里时，它和端口、
              访问日志、令牌挤在同一屏，而名单的判定顺序又是跨页的 ——
              想知道某个地址会不会被拦，得先在脑子里拼三处配置。
              现在统一在「IP 名单」页签维护，这里只留一句指路。
            */}
            <Note kind="info" span>
              <b>IP 名单（全局黑名单 / 路由白名单 / 路由黑名单）已移到「IP 名单」页签。</b>
              <br />
              三份名单的判定顺序是固定的：全局黑名单 → 路由白名单 → 路由黑名单 → 放行。
              那一页里还带一个<b>命中测试</b>：填个地址就能看出它会被哪一层、哪条规则拦下。
            </Note>
          ...
          <Note kind="info">
            ...
            这个接口是合并语义，只提交上面这几个字段 —— 名单（在「IP 名单」页签改）
            不会被这里的一次保存碰到。
          </Note>
        </div>
      </Card>
