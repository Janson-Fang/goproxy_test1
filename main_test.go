...
// 配置目录的「可写性探针」。
//
// 管理接口改配置要写数据库（SQLite 在写入时会建 -wal / -shm 边车文件，
// 以及写入时的临时文件）。目录不可写时增删改路由会全部失败，但失败只在用户
// 真正点下去那一刻才暴露 —— 报的是「attempt to write a readonly database」，
// 看起来像接口 bug，实际是部署配置问题。启动时探一次并打日志，把这两件事区分开。
...
func TestConfigWriteProbeDetectsUnwritableDir(t *testing.T) {
	...
	// 目录不存在 → 建不出临时文件，与「目录只读」在效果上等价
	t.Run("目录不存在应报错", func(t *testing.T) {
		a := &App{configDB: filepath.Join(dir, "missing-dir", "goproxy.db")}
		if err := a.configWriteProbe(); err == nil {
